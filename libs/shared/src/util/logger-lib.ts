import { json } from 'libs/shared/src/util/parser/json';
import { _json5 } from 'libs/shared/src/util/parser/json5';
...
const logMessage = (level: LoggerInterface['level'], moduleFile: string, message: any) => {
  ...
  return logger(`[${level}] [${initial}] [${moduleFile.replace(wd, '').replace(/\\/gu, '/')}] [${_json5.stringify(message)}]`);
};
...
const logError = (level: LoggerInterface['level'], moduleFile: string, message: any[]) =>
  logger(
    `[${level}] [${moduleFile.replace(wd, '').replace(/\\/gu, '/')}] [${_json5.stringify(
